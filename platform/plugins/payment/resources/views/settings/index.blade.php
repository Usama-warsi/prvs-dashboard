        ...
        <div class="col-12 col-md-9">
            ...
            <x-core::card class="mb-3">
                <x-core::table :hover="false" :striped="false">
                    ...
                    @php
                        $Paylaterstatus = setting('payment_pay_later_status');
                    @endphp
                    <x-core::table.body>
                        <x-core::table.body.row>
                            <x-core::table.body.cell colspan="3">
                                    ...
                                    <div>
                                        <div class="payment-name-label-group">
                                            @if($PaylaterForm)
                                                {{ trans('plugins/payment::payment.use') }}
                                            @endif
                                            <span class="method-name-label">{{ get_payment_setting('name', 'pay_later', PaymentMethodEnum::PAY_LATER()->label()) }}</span>
                                        </div>
                                    </div>

                                    <x-core::button @class(['toggle-payment-item edit-payment-item-btn-trigger', 'hidden' => !$Paylaterstatus])>
                                        {{ trans('plugins/payment::payment.edit') }}
                                    </x-core::button>
                                    <x-core::button @class(['toggle-payment-item save-payment-item-btn-trigger', 'hidden' => $Paylaterstatus])>
                                        {{ trans('plugins/payment::payment.settings') }}
                                    </x-core::button>
                                </div>
                            </x-core::table.body.cell>
                        ...
                        <x-core::table.body.row class="payment-content-item hidden">
                            <x-core::table.body.cell colspan="3">
                                <x-core::form>
                                    {!! $PaylaterForm->renderForm() !!}

                                    <div class="btn-list justify-content-end">
                                        <x-core::button
                                            type="button"
                                            @class(['disable-payment-item', 'hidden' => !$Paylaterstatus])
                                        >
                                            {{ trans('plugins/payment::payment.deactivate') }}
                                        </x-core::button>

                                        <x-core::button
                                            @class(['save-payment-item btn-text-trigger-save', 'hidden' => $Paylaterstatus])
                                            type="button"
                                            color="info"
                                        >
                                            {{ trans('plugins/payment::payment.activate') }}
                                        ...
                                        <x-core::button
                                            type="button"
                                            color="info"
                                            @class(['save-payment-item btn-text-trigger-update', 'hidden' => !$Paylaterstatus])
                                        >
                                            {{ trans('plugins/payment::payment.update') }}
                                        </x-core::button>
                                    </div>
