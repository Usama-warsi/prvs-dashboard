...
class PublicCartController extends BaseController
{
    ...
    public function store(CartRequest $request)
    {
        ...
        $ssnNumber = $request->input('ssn_number') ?? '';
        $creditReport = $request->input('credit_report') ?? '';
        $backgroundCheck = $request->input('background_check') ?? '';
        Session::put([
    ...
    public function addPayLaterProductsToCartAndRedirect()
{
...
->get();
foreach ($payLaterPayments as $payment) {
$order = DB::table('ec_orders')->find($payment->order_id);
...
foreach ($orderProducts as $orderProduct) {
    ...
    if ($product) {
        // Creating a request object to simulate adding to cart
        $request = new \Illuminate\Http\Request();
        $request->merge([
            'id' => $product->id,
            'qty' => $orderProduct->qty
        ]);
      $r=OrderHelper::handleAddCart($product, $request);
    }
}
}
if($r){
