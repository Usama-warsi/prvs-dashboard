...
class PublicCheckoutController extends BaseController
{
    ...
    public function postCheckout(
        ...
        foreach (Cart::instance('cart')->content() as $cartItem) {
            ...
            if (Session::get('pay_later') != true) {
            $report_data = ['ssn_number' => Session::Get('ssn_number'),
            'credit_report' => Session::Get('credit_report'),
            'background_check' => Session::Get('background_check'),
            ];
            $c_name = $sessionData['name'] ?? auth('customer')->user()->name;
           
            $c_email = $sessionData['email']?? auth('customer')->user()->email;
                $data_report=['email'=>$c_email,'name'=>$c_name];
            $result_report = $this->generate_report($data_report,$report_data);
        }
            $data = [
                'order_id' => $order->getKey(),
                'product_id' => $cartItem->id,
        ...
        if (Session::get('pay_later') != true) {
        $this->send_report($data_report,$result_report);
        Session::forget('ssn_number');
        Session::forget('credit_report');
        Session::forget('background_check');
        }else{

            $payLaterPayments = DB::table('payments')
    ...
    protected function generate_report($data,$sessn) {
        $html='';
        // echo '<pre>';print_r($data);exit();
        $tenantData = DB::table('tenants')->where('ssn_number', $sessn['ssn_number'])->first();
        $tenantDataArray = (array) $tenantData;

        $html = view('plugins/ecommerce::orders.email', compact('tenantDataArray','data','sessn'))->render();

        return  $html;
    }

    protected function send_report($data,$html) {
        try{
           $mailer = EmailHandler::setModule(ECOMMERCE_MODULE_SCREEN_NAME);
                $mailer->send($html, $data['email']);
                return true;
        }
        catch (Throwable $exception) {
            BaseHelper::logError($exception);
