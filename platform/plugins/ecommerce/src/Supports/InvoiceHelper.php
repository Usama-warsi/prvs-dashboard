...
class InvoiceHelper
{
    ...
    public function makereportPDF($html): PDFHelper
    {
        return (new Pdf())
            ->templatePath($this->getreportTemplatePath())
            ->destinationPath($this->getreportTemplateCustomizedPath())
            ->paperSizeA4()
            ->data(['data' => $html])
            ->twigExtensions([
                new TwigExtension(),
    ...
    public function getreportTemplatePath(): string
    {
        return plugin_path('ecommerce/resources/templates/report_request.tpl');
    }
    public function getreportTemplateCustomizedPath(): string
    {
        return storage_path('app/templates/ecommerce/report_request.tpl');
    }
    public function downloadreport($html)
    {
        return $this->makereportPDF($html)->download(sprintf('report.pdf'));
    }
}
